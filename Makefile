...
KiCad/PN532-Round.scad: KiCad/PN532-Round.kicad_pcb PCBCase/case Makefile
	...
	@echo "base();" >> $@
	@echo "translate([spacing,0,0])difference(){top();translate([casewall+23,casewall+23,-1]){hull()for(y=[0,-3.44])translate([0,y,0])cylinder(d=3.5,h=2);hull()for(y=[0,-3.44])translate([0,y,0.2])cylinder(d1=3.5,d2=7,h=3.5);}}" >> $@
